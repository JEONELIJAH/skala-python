...
def run_basic_statistics_and_ttest(df):
    ...
    if p_val < 0.05:
        ...
        print("해석: p-value >= 0.05 이므로, 카드와 현금 결제 간 평균 이동 거리 차이는 통계적으로 유의미하지 않습니다. (귀무가설 채택)")
        ttest_verdict = "유의미한 차이 없음 (귀무가설 채택)"
...
def analyze_dispute_zones(df, zone_lookup_path):
    ...
    try:
        ...
        zones = pd.read_csv(zone_lookup_path).set_index("LocationID")[["Borough", "Zone"]]
        zone_stats = zone_stats.join(zones, how="left")
        ...
        logger.error(f"가설 2 검증 중 오류 발생: {e}")
        return None
...
if __name__ == "__main__":
    ...
    # [2] 분쟁 지역 가설 검증
    dispute_result = analyze_dispute_zones(df, ZONE_LOOKUP_PATH)
